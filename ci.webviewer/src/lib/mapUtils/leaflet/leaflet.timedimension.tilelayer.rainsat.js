/*
 * L.TimeDimension.Layer.TileLayer.Rainsat: TimeDimension TileLayer for Rainsat.
 */
...
L.TimeDimension.Layer.TileLayer.Rainsat = L.TimeDimension.Layer.TileLayer.extend({
    ...
    _createLayerForTime:function(time){
        ...
        var d  = new Date(time).toISOString().substring(0,19).replace(/-/g, '').replace(/:/g, '');
        url = url.replace('{d}', d );
        return new this._baseLayer.constructor(url, options);
    },
...
L.timeDimension.layer.tileLayer.rainsat = function(layer, options) {
    return new L.TimeDimension.Layer.TileLayer.Rainsat(layer, options);
};